  ...
  methods: {
    ...
    update_links: function(event) {
      ...
      while (j < this.nodes.length - 1) {
        while (this.nodes[i].selected && i < this.nodes.length - 2) {
          i++;
        }
        // We reached the end of the array. Stop
        ...
        while (this.nodes[j].selected && j < this.nodes.length - 1) {
          j++;
        }
